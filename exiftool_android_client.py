...
NIKON_Z_LENS_MAP = {
    ...
    4: "Nikkor Z 85mm f/1.8 S",
    5: "Nikkor Z 14-30mm f/4 S",
    6: "Nikkor Z 24-70mm f/2.8 S",
    7: "Nikkor Z 14-24mm f/2.8 S",
    8: "Nikkor Z 20mm f/1.8 S",
    9: "Nikkor Z 24mm f/1.8 S",
    10: "Nikkor Z 35mm f/1.2 S",
    11: "Nikkor Z 50mm f/1.2 S",
    12: "Nikkor Z 85mm f/1.2 S",
    13: "Nikkor Z 105mm f/2.8 VR S Macro",
    14: "Nikkor Z 28-75mm f/2.8",
    15: "Nikkor Z 180-600mm f/5.6-6.3 VR",
    16: "Nikkor Z 70-200mm f/2.8 VR S",
    17: "Nikkor Z 100-400mm f/4.5-5.6 VR S",
    18: "Nikkor Z 24-120mm f/4 S VR"
}
VENDOR_LENS_TAG_MAP = {
    "NIKON": ["LensID","LensType", "Lens", "MakerNoteNikon_LensType", "LensModel", "Lens ID"],
    "CANON": ["LensType", "Lens", "MakerNoteCanon_LensType", "LensModel"],
    "FUJIFILM": ["MakerNoteFuji_LensName", "Lens", "LensModel"],
    "SONY": ["MakerNoteSony_LensSpec", "Lens", "LensModel"],
    "OLYMPUS": ["MakerNoteOlympus_LensInfo", "Lens"],
    "LEICA": ["MakerNoteLeica_LensID", "Lens"],
    "PANASONIC": ["MakerNotePanasonic_LensType", "Lens"]
}
...
def draw_icon(draw, x, y, icon_type, line_color=(255, 255, 255)):
    ...
    if icon_type == "shutter_lightning":
        ...
        seg1 = [(center_x - scale*0.5/1.732+head_r, center_y + scale*0.5), (center_x + scale*0.25/1.732+head_r, center_y -  scale*0.25)]
        draw.line(seg1, fill=col, width=line_w)
        seg3 = [(center_x+head_r-1/12*scale, center_y+1/12*scale*1.732), (center_x + scale/6+head_r, center_y + scale/4)]
        draw.line(seg3, fill=col, width=line_w)
        seg2 = [ (center_x + scale/6+head_r, center_y + scale/4), (center_x + scale/6-scale/8/0.577+head_r, center_y + scale*(1/3+1/8))]
        draw.line(seg2, fill=col, width=line_w)
        seg4 = [(center_x +head_r+ scale*(0.25/1.732+1/4/1.732), center_y + scale*(-1/4+1/4)), (center_x +head_r+ scale*(0.25/1.732+1/4/1.732+1/6/0.577), center_y + scale*(-1/4+1/4 -1/6))]
        draw.line(seg4, fill=col, width=line_w)
        seg5 = [(center_x + scale*0.25/1.732+head_r, center_y -  scale*0.25), (center_x +head_r+ scale*(0.25/1.732+1/4/1.732), center_y + scale*(-1/4+1/4))]
        draw.line(seg5, fill=col, width=line_w)
        seg6 = [(center_x + scale*0.25/1.732+head_r, center_y - scale*0.25), (center_x + scale*(0.25/1.732-1/5)+head_r, center_y - scale*0.25)]
        draw.line(seg6, fill=col, width=line_w)
        seg7 = [ (center_x + scale*(0.25/1.732-1/5)+head_r, center_y - scale*0.25),  (center_x + scale*(0.25/1.732-1/5-1/6)+head_r, center_y + scale*(-0.25+1/6*1.732))]
        ...
        line_w=LINE_WIDTH
        col=line_color
        seg1 = [(center_x - scale*0.5, center_y + scale*0.35/2), (center_x - scale*0.5, center_y - scale*0.35/2)]
        draw.line(seg1, fill=col, width=line_w)
        seg2 = [(center_x - scale*(0.5-1/66), center_y + scale*0.37/2), (center_x - scale*(0.5-1/66), center_y - scale*0.37/2)]
        draw.line(seg2, fill=col, width=line_w)
        seg3 = [(center_x - scale*55/200, center_y + scale*0.48/2), (center_x - scale*55/200, center_y - scale*0.48/2)]
        draw.line(seg3, fill=col, width=line_w)
        seg4 = [(center_x - scale*52/200, center_y + scale*0.48/2), (center_x - scale*52/200, center_y - scale*0.48/2)]
        draw.line(seg4, fill=col, width=line_w)
        seg5 = [(center_x - scale*21/200, center_y + scale*0.48/2), (center_x - scale*21/200, center_y - scale*0.48/2)]
        draw.line(seg5, fill=col, width=line_w)
        seg6 = [(center_x - scale*19/200, center_y + scale*0.48/2), (center_x - scale*19/200, center_y - scale*0.48/2)]
        draw.line(seg6, fill=col, width=line_w)
        seg7 = [(center_x - scale*17/200, center_y + scale*0.48/2), (center_x - scale*17/200, center_y - scale*0.48/2)]
        draw.line(seg7, fill=col, width=line_w)
        seg8 = [(center_x + scale*25/200, center_y + scale*0.48/2), (center_x + scale*25/200, center_y - scale*0.48/2)]
        draw.line(seg8, fill=col, width=line_w)
        seg9 = [(center_x + scale*28/200, center_y + scale*0.48/2), (center_x + scale*28/200, center_y - scale*0.48/2)]
        draw.line(seg9, fill=col, width=line_w)
        seg10 = [(center_x - scale*0.5, center_y + scale*0.35/2), (center_x - scale*55/200, center_y + scale*0.48/2)]
        draw.line(seg10, fill=col, width=line_w)
        seg11 = [(center_x - scale*0.5, center_y - scale*0.35/2), (center_x - scale*55/200, center_y - scale*0.48/2)]
        draw.line(seg11, fill=col, width=line_w)
        seg12 = [(center_x - scale*0.55/2, center_y - scale*0.48/2), (center_x + scale*25/200, center_y - scale*0.48/2)]
        draw.line(seg12, fill=col, width=line_w)
        seg13 = [(center_x - scale*0.55/2, center_y + scale*0.48/2), (center_x + scale*25/200, center_y + scale*0.48/2)]
        draw.line(seg13, fill=col, width=line_w)
        seg14 = [(center_x + scale*25/200, center_y + scale*0.48/2), (center_x + scale*81/200, center_y + scale*0.81/2)]
        draw.line(seg14, fill=col, width=line_w)
        seg15 = [(center_x + scale*25/200, center_y - scale*0.48/2), (center_x + scale*81/200, center_y - scale*0.81/2)]
        draw.line(seg15, fill=col, width=line_w)
        seg16 = [(center_x + scale*63/200, center_y + scale*0.48/2), (center_x + scale*81/200, center_y + scale*0.81/2)]
        draw.line(seg16, fill=col, width=line_w)
        seg17 = [(center_x + scale*63/200, center_y - scale*0.48/2), (center_x + scale*81/200, center_y - scale*0.81/2)]
        draw.line(seg17, fill=col, width=line_w)
        seg18 = [(center_x + scale*63/200, center_y + scale*0.48/2), (center_x + scale*81/200, center_y )]
        draw.line(seg18, fill=col, width=line_w)
        seg19 = [(center_x + scale*63/200, center_y - scale*0.48/2), (center_x + scale*81/200, center_y )]
...
class ExifWatermarkRoot(BoxLayout):
    ...
    def action_open(self, instance):
        ...
        filechooser.open_file(
            filters=["*.jpg", "*.jpeg", "*.nef", "*.arw", "*.raf", "*.cr2", "*.orf", "*.rw2", "*.pef", "*.x3f", "*.dng", "*.srw"],
    ...
    def on_file_selected(self, selection):
        ...
        self.raw_pil_img = Image.open(self.img_path).convert("RGBA")
        self.rotate_angle = 0
        self.render_cache = []
        self.refresh_preview()
        # 后台线程上传伪造图片获取EXIF
        threading.Thread(target=self.read_exif_remote, args=(self.img_path,), daemon=True).start()

    def read_exif_remote(self, img_path):
        try:
            # 安卓临时目录
            tmp_dir = storage.get_cache_dir()
            self.temp_fake_jpg = os.path.join(tmp_dir, "temp_fake.jpg")
            generate_empty_fake_jpg_with_raw_exif(img_path, self.temp_fake_jpg)
            files = {"file": (os.path.basename(self.temp_fake_jpg), open(self.temp_fake_jpg, "rb"))}
    ...
    def draw_watermark_on_image(self, pil_img):
        ...
        if not data:
            ...
            return pil_img
        pad, ls = cfg["padding"], cfg["line_spacing"]
        pad=BOX_PADDING
        ls=LINE_SPACING
        ...
        icon_gap = ICON_SIZE + ICON_TEXT_GAP
    ...
    def action_save(self, instance):
        if self.processed_pil_img is None:
            return
        # 安卓公共图片目录
        save_dir = storage.get_documents_dir()
        save_name = "exif_watermark.jpg"
        self.output_save_path = os.path.join(save_dir, save_name)
        rgb_img = self.processed_pil_img.convert("RGB")
        rgb_img.save(self.output_save_path, quality=95)

    def action_share(self, instance):
        if not self.output_save_path or not os.path.exists(self.output_save_path):
            self.action_save(None)
        # 调用系统原生分享面板，用户自行选择微信好友
        sharing.share_file(path=self.output_save_path, title="分享图片")
